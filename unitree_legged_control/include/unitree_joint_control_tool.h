...
double computeTorqueLarge(double current_position, double current_velocity, ServoCmd &cmd)
{
	double tau;
	tau = cmd.posStiffness * (cmd.pos - current_position) + cmd.velStiffness * (cmd.vel - current_velocity) + cmd.torque;
	return tau;
}
